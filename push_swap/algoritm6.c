...
int lists_lenght;
	...
	search =  find_center(*beta, ft_list_size(*beta)/count);// вот это нужно оптимизировать. как-то
	while(ft_list_size(*beta)!= 0)
	{
		searcher2 = *beta;
		stop_sort = step;
		//search =  find_center(*beta, ft_list_size(*beta)/count);
		if(step2 >= step)
		{
			while (stop_sort >= 0)
			{
...
int lists_lenght;
	...
	search =  find_center(*alpha, ft_list_size(*alpha)/count );// вот это нужно оптимизировать. как-то
	while(ft_list_size(*alpha) != 0)
	{
			searcher2 = *alpha;
			stop_sort = step;

			//search =  find_center(*alpha, ft_list_size(*alpha)/count );
			if(step2 >= step)
			{
			while (stop_sort >= 0)
			{
